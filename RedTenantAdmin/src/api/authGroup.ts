...
export const getAuthGroups = (data: AuthGroupSearch) => {
  return http.request<AuthGroupListResult>("post", "/api/v1/tenant/authGroup/list", { data });
};

/** 创建或更新授权群组 */
export const setAuthGroup = (data: AuthGroup) => {
  return http.request<AuthGroupResult>("post", "/api/v1/tenant/authGroup", { data });
};

/** 删除授权群组 */
export const delAuthGroup = (id: number) => {
  return http.request<any>("delete", `/api/v1/tenant/authGroup/${id}`);
};
