...
func GetShardingTable(tableName string, fieldValue interface{}, isArray bool) string {
	if isArray {
		...
		for i := 0; i < val.Len(); i++ {
			...
			if len(tableNames) > 0 && !utils.InStrings(tableNames, tempName) {
...
func BeforeCudHook(tx *gorm.DB) {
	if prefixTables[tx.Statement.Table] {
		prefix := ""
		prefixObj := tx.Statement.Context.Value(utils.KeyTablePrefix)
		if prefixObj != nil && !strings.HasPrefix(tx.Statement.Table, prefixObj.(string)) {
			prefix = prefixObj.(string)
		}
		tx.Statement.Table = fmt.Sprintf("%s_%s", prefix, tx.Statement.Table)
	}
