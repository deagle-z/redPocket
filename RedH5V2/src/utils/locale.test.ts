...
describe('locale utilities', () => {
  ...
  it('resolves the first supported non-Chinese browser language', () => {
    ...
    expect(resolveBrowserLocale(['pt-BR', 'es-ES', 'en-US'])).toBe('es-MX')
  ...
  it('falls back to Mexican Spanish for unsupported or Chinese-only browser languages', () => {
    expect(resolveBrowserLocale(['fr-FR', 'ja-JP'])).toBe('es-MX')
    expect(resolveBrowserLocale(['zh-CN', 'zh-Hans-CN'])).toBe('es-MX')
    expect(resolveBrowserLocale([])).toBe('es-MX')
  })

  it('defaults initial locale to Mexican Spanish when no manual locale is stored', () => {
    expect(getInitialLocale(['zh-CN'])).toBe('es-MX')
  })
})
