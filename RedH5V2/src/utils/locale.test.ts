...
describe('locale utilities', () => {
  ...
  it('resolves the first supported non-Chinese browser language', () => {
    ...
    expect(resolveBrowserLocale(['pt-BR', 'es-ES', 'en-US'])).toBe('es-US')
    expect(resolveBrowserLocale(['zh-CN', 'en-US'])).toBe('en-US')
  })

  it('falls back to English for unsupported or Chinese-only browser languages', () => {
    expect(resolveBrowserLocale(['fr-FR', 'ja-JP'])).toBe('en-US')
    expect(resolveBrowserLocale(['zh-CN', 'zh-Hans-CN'])).toBe('en-US')
    expect(resolveBrowserLocale([])).toBe('en-US')
  })

  it('defaults initial locale to English when no manual locale is stored', () => {
    expect(getInitialLocale(['zh-CN'])).toBe('en-US')
  })
})
