...
const balanceText = computed(() => {
  return formatMoney(toDisplayCents(selectedRechargeWalletBalance.value), { currency: '$' })
})
...
function persistRechargeFieldValues() {
  if (isHydratingRechargeFields.value || !rechargeFields.value.length) return
...
async function loadRechargePage() {
  ...
  try {
    ...
    selectedCountry.value = countries.value.find(country => country.countryCode === US_COUNTRY_CODE) ?? null
    ...
    channels.value = fixedRechargeChannels()
...
onMounted(() => {
  void loadRechargePage()
})
</script>

<template>
  <main class="ppmx-page ppmx-subpage ppmx-recharge-page">
    <PpmxPageChrome>
      <section class="ppmx-subpage__wide ppmx-recharge-stack">
        <AppState
          v-if="pageStatus !== 'ready'"
          class="ppmx-recharge-state"
          :status="pageStatus"
          :title="pageError || undefined"
          :message="pageStateMessage"
          :action-text="t('common.retry')"
          skeleton-variant="recharge-page"
          @retry="loadRechargePage"
        />

        <section v-else id="depositModal" class="ppmx-recharge-modal">
          <div class="ppmx-recharge-modal__backdrop" aria-hidden="true" />
          <section class="ppmx-recharge-modal__card ppmx-glass-strong reveal" :aria-label="t('recharge.title')">
            <button
            ...
            <div class="ppmx-recharge-modal__scroll dep-scroll">
              <header class="ppmx-recharge-modal__head">
                <h1>{{ t('recharge.title') }}</h1>
                <p>{{ t('recharge.description') }}</p>
              </header>
              ...
              <section class="ppmx-recharge-panel ppmx-recharge-form">
                ...
                <section v-if="rechargeFields.length" class="ppmx-recharge-section ppmx-recharge-section--fields">
                  <div class="ppmx-recharge-panel__head">
                ...
                <section v-if="showPromo" class="ppmx-recharge-section ppmx-recharge-promo">
                  ...
                  <div class="ppmx-recharge-promo__list">
                    <article
                      class="ppmx-recharge-promo-card"
              ...
              <section class="ppmx-recharge-rules">
                <h3>
