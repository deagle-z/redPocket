...
function normalizeTaskActivities(value: TaskActivityItem[] | undefined | null) {
  ...
  return Array.isArray(value)
    ? value.filter(item => Number(item?.configId) > 0 || Number(item?.recordId) > 0)
...
async function claimTeamTask() {
  ...
  try {
    const claimed = await claimTaskActivity(currentLang.value)
...
<template>
  <main id="page-team-v2" class="ppmx-page ppmx-subpage ppmx-team-v2-page">
          ...
          <section id="teamV2TaskCard" class="ppmx-team-v2-task reveal in">
            ...
            <div v-if="taskActivities.length > 1" id="teamV2TaskTiers" class="ppmx-team-v2-task__tiers">
              <button
                ...
                type="button"
                :class="{ 'is-active': taskTier === index }"
                @click="taskTier = index"
              >
                <span>{{ taskLevelText(tier) }}</span>
                <small>{{ formatCount(tier.requiredInviteCount) }} {{ t('ppmx.team.statValidUsers') }}</small>
              </button>
            </div>

            <article v-if="activeTask" class="ppmx-team-v2-task__card">
              <span class="ppmx-team-v2-task__icon">
                <i class="fa-solid fa-user-group" />
              </span>
              <div>
                <strong>{{ t('ppmx.team.taskInviteRewardTitle') }}</strong>
            ...
            <article v-else class="ppmx-team-v2-task__card ppmx-team-v2-task__card--empty">
              <span class="ppmx-team-v2-task__icon">
                <i class="fa-solid fa-bullseye" />
              </span>
              <div>
