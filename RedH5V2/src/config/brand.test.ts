...
const activeAppFiles = [
  ...
  new URL('../locales/es-US.ts', import.meta.url),
...
describe('PP.BET brand foundation', () => {
  it('exposes the project brand contract', () => {
    expect(APP_BRAND.name).toBe('PP.BET')
    expect(APP_BRAND.shortName).toBe('PP.BET')
  ...
  it('renders a branded initialization loading screen before Vue mounts', () => {
    ...
    expect(indexSource).toContain('app-loading__brand">PP<span>.</span>BET')
