...
async function loadPacketList() {
  ...
  try {
    ...
    const { data } = await getTrialLuckyList({
      currentPage: 0,
      pageSize: 20,
...
async function grabTrialPacket(data: { luckyId: number, grabIndex?: number, oddEvenGuess?: 0 | 1 }) {
  const response = await grabTrialLucky(data)
  response.data = {
    ...response.data,
    gameMode: isParityPlayType(pendingGrabTarget.value?.packet?.playType) ? 1 : 0,
    guess: data.oddEvenGuess,
  } as any
  return response
}
...
function applyLuckyBroadcast(message: any) {
  const lucky = message?.data || message
  const luckyId = Number(lucky?.id || lucky?.luckyId || 0)
  ...
  packetList.value = packetList.value.map((packet) => {
    ...
    if (nextStatus === 'ongoing') {
      if (grabbedSeqNo > 0) {
        ...
        if (idx >= 0) {
          const nextAmount = Number(lucky?.grabAmount ?? lucky?.amount ?? nextActions[idx]?.amount ?? 0)
          nextActions[idx] = {
            ...nextActions[idx],
            isGrabbed: true,
...
function applyLuckyFinished(message: any) {
  ...
  const participants: any[] = detail?.participants || detail?.items || []
  ...
  packetList.value = packetList.value.map((packet) => {
    ...
    const updatedActions = existingActions.map((action: any) => {
      ...
      return {
        ...
        isGrabbed: grabbed,
        amount,
        thunder: grabbed ? Number(participant.isThunder ?? participant.thunder ?? 0) : 0,
...
async function loadCurrentUserBalance() {
  ...
  try {
    const { data } = await getTrialMe()
    const balance = Number(data?.trialBalance ?? 0)
    currentUserBalance.value = balance
  }
  catch {
    currentUserBalance.value = 0
  }
...
onBeforeUnmount(() => {
  ...
  wsClient.off('trial_lucky_sent', applyLuckySent)
  wsClient.off('trial_lucky_grabbed', applyLuckyBroadcast)
  wsClient.off('trial_lucky_finished', applyLuckyFinished)
})
</script>
