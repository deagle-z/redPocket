...
func (g *Provider) CreateOrder(req pay.PayRequest) (pay.PayResponse, error) {
	...
	if cfg.MerNo == "" || cfg.Secret == "" {
		return pay.PayResponse{}, fmt.Errorf("GCTPKMXN 配置不完整，请检查 core.yaml pay.gctpkmxn 节点")
	}
	...
	if err != nil {
		return pay.PayResponse{}, fmt.Errorf("GCTPKMXN 请求失败: %w", err)
	}

	var apiResp createOrderResp
	if err = json.Unmarshal(respBody, &apiResp); err != nil {
		return pay.PayResponse{}, fmt.Errorf("GCTPKMXN 响应解析失败: %w", err)
	}
	if apiResp.Code != 200 {
...
func (g *Provider) CreatePayoutOrder(req pay.PayoutRequest) (pay.PayoutResponse, error) {
	...
	if identityType == "" {
		return pay.PayoutResponse{}, fmt.Errorf("GCTPKMXN 代付参数缺少 identityType")
	}
