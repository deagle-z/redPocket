...
// 抢包流水：SUM(amount + lose_money) WHERE user_id=? AND grab_type 非中雷返还
...
func GetUserTotalFlow(db *gorm.DB, userID int64) (float64, error) {
	...
	if err := db.Raw(`
		SELECT
			COALESCE((SELECT SUM(amount + lose_money) FROM lucky_history WHERE user_id = ? AND COALESCE(grab_type, 0) != 2), 0) +
