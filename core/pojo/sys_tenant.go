...
type SysTenant struct { // 租户主表
	...
	EnableWithdraw               int        `json:"enableWithdraw" gorm:"column:enable_withdraw;type:tinyint;default:0"`                                      // 是否开启提现审核：0=关闭 1=开启
	WithdrawAutoReviewEnabled    int        `json:"withdrawAutoReviewEnabled" gorm:"column:withdraw_auto_review_enabled;type:tinyint;default:0"`              // 是否开启提现自动审核：0=关闭 1=开启
	WithdrawAutoReviewMaxAmount  float64    `json:"withdrawAutoReviewMaxAmount" gorm:"column:withdraw_auto_review_max_amount;type:numeric(18,2);default:0"`   // 自动审核单笔金额上限
	WithdrawAutoReviewDailyLimit float64    `json:"withdrawAutoReviewDailyLimit" gorm:"column:withdraw_auto_review_daily_limit;type:numeric(18,2);default:0"` // 自动审核每日全局总额上限
	...
	BindDomain                   *string    `json:"bindDomain" gorm:"column:bind_domain;type:varchar(128);uniqueIndex"`                                       // 绑定域名
	TgServiceURL                 *string    `json:"tgServiceUrl" gorm:"column:tg_service_url;type:varchar(255)"`                                              // TG客服链接
	WsServiceURL                 *string    `json:"wsServiceUrl" gorm:"column:ws_service_url;type:varchar(255)"`                                              // WS客服链接
...
type SysTenantSearch struct {
	...
	BindDomain  string `json:"bindDomain"`  // 绑定域名
}
...
type SysTenantSet struct {
	...
	WithdrawAutoReviewEnabled    int     `json:"withdrawAutoReviewEnabled"`
	WithdrawAutoReviewMaxAmount  float64 `json:"withdrawAutoReviewMaxAmount"`
	WithdrawAutoReviewDailyLimit float64 `json:"withdrawAutoReviewDailyLimit"`
	...
	WsServiceURL                 *string `json:"wsServiceUrl"`
...
type SysTenantServiceLinksBack struct {
	TgServiceURL *string `json:"tgServiceUrl"`
	WsServiceURL *string `json:"wsServiceUrl"`
}
