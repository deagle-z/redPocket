...
type ExchangeCode struct {
	...
	BatchNo        string  `json:"batchNo" gorm:"column:batch_no;type:varchar(32);not null;default:'';index;comment:批次号（同批次一个用户仅可兑换一次，空表示不限批次）"`
	Tag            string  `json:"tag" gorm:"column:tag;type:varchar(64);not null;default:'';index;comment:标签（同标签一个用户仅可兑换一次，空表示不限标签）"`
...
type ExchangeCodeRedeem struct {
	...
	BatchNo         string  `json:"batchNo" gorm:"column:batch_no;type:varchar(32);not null;default:'';index:idx_exchange_code_redeem_batch_user,priority:1;comment:批次号"`
	Tag             string  `json:"tag" gorm:"column:tag;type:varchar(64);not null;default:'';index:idx_exchange_code_redeem_tag_user,priority:1;comment:标签"`
	UserID          int64   `json:"userId" gorm:"column:user_id;type:bigint;not null;uniqueIndex:idx_exchange_code_redeem_once,priority:2;index;index:idx_exchange_code_redeem_batch_user,priority:2;index:idx_exchange_code_redeem_tag_user,priority:2;comment:用户ID"`
