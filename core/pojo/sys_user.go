...
type UserAdd struct {
	ID       int64    `json:"id"`                                             // ID
	Username string   `json:"username" binding:"required,min=6,max=64"`       // 用户名，必填，长度6-64字符
	Password string   `json:"password"`                                       // 密码，长度8-64字符
	Enabled  bool     `json:"enabled"`                                        // 是否可用，必填
...
type UserPassChange struct {
	ID       int64  `json:"id" binding:"required,gt=0"`               // ID
	Password string `json:"password" binding:"required,min=6,max=64"` // 密码，长度8-64字符
}
