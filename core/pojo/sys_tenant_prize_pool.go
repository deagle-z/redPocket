...
type SysTenantPrizePool struct {
	...
	Balance       float64   `json:"balance" gorm:"type:decimal(18,2);not null;default:0"`       // 当前奖池余额
	FrozenBalance float64   `json:"frozenBalance" gorm:"type:decimal(18,2);not null;default:0"` // 冻结金额（已开奖未结算）
	MinBalance    float64   `json:"minBalance" gorm:"type:decimal(18,2);default:0"`             // 最低安全值（低于触发补池）
	MaxBalance    *float64  `json:"maxBalance" gorm:"type:decimal(18,2);default:null"`          // 最高限制（超过可分红/抽水），NULL=不限
