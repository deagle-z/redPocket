...
func FlushTempHostInfo() {
	...
	Db.Find(&hostInfos)
	if len(hostInfos) > 0 {
		jsonStr, _ := json.Marshal(hostInfos)
		RD.SetEX(context.Background(), KeyHostInfosTemp, string(jsonStr), GetRandomRangeSecond(20*60, 40*60))
	}
...
func GetTempHostInfos() (result []pojo.HostInfo) {
	result, _ = GetOrLoad(context.Background(), Db, KeyHostInfosTemp, GetRandomRangeSecond(20*60, 40*60),
		func(db *gorm.DB) (tempResult []pojo.HostInfo, err error) {
			if err = Db.Find(&tempResult).Error; err != nil {
				return tempResult, err
			}
			if len(tempResult) == 0 {
				err = errors.New("no match data")
