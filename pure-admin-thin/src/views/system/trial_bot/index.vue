...
import {
  batchCreateTrialBots,
  batchUpdateTrialBots,
  type TrialBotUser
} from "@/api/trialBot";
import { useTrialBotUser } from "./utils/hook";
...
function resetBatchCreateForm() {
  ...
  batchCreateForm.balance = 1000000;
...
function resetBatchEditForm() {
  ...
  batchEditForm.balanceEnabled = false;
  batchEditForm.balance = 1000000;
...
function openBatchEditDialog() {
  if (selectedRows.value.length === 0) {
    message("请先勾选要修改的试玩机器人", { type: "warning" });
    return;
  }
  resetBatchEditForm();
...
async function submitBatchCreate() {
  ...
  try {
    const { data } = await batchCreateTrialBots({
      ...
      balance: batchCreateForm.balance
    });
    message(`已生成 ${data?.count || 0} 个试玩机器人`, { type: "success" });
    createDialogVisible.value = false;
    onSearch();
  } catch (error) {
    console.error("批量生成试玩机器人失败", error);
    message("批量生成试玩机器人失败", { type: "error" });
  } finally {
...
async function submitBatchUpdate() {
  const ids = selectedRows.value.map((item: TrialBotUser) => Number(item.id));
  if (ids.length === 0) {
    message("请先勾选要修改的试玩机器人", { type: "warning" });
    return;
  }
  ...
  const shouldUpdateBalance = batchEditForm.balanceEnabled;
  if (
    !shouldUpdateName &&
    !shouldUpdateAvatar &&
    !shouldUpdateStatus &&
  ...
  try {
    ...
    if (shouldUpdateBalance) {
      payload.balance = batchEditForm.balance;
    }
    const { data } = await batchUpdateTrialBots(payload);
    message(`已批量修改 ${data?.count || 0} 个试玩机器人`, {
      type: "success"
    });
    ...
    console.error("批量修改试玩机器人失败", error);
    message("批量修改试玩机器人失败", { type: "error" });
  } finally {
    submitLoading.value = false;
  }
}
    ...
    <div :class="[deviceDetection() ? ['w-full', 'mt-2'] : 'w-[calc(100%)]']">
        ...
        <template v-slot="{ size, dynamicColumns }">
          ...
          >
            <template #operation="{ row }">
              ...
              <el-popconfirm
                :title="`是否确认删除试玩机器人编号为${row.id}的这条数据`"
      ...
      >
        <el-form :inline="true" :model="batchCreateForm" class="search-form">
          ...
          <el-form-item label="初始试玩余额">
            <el-input-number
              v-model="batchCreateForm.balance"
              :min="0"
              :step="1000"
            />
          </el-form-item>
      ...
      >
        <el-form :inline="true" :model="batchEditForm" class="search-form">
          <el-form-item label="已选数量">
            <el-tag type="primary">{{ selectedNum }} 个试玩机器人</el-tag>
          </el-form-item>
          <el-form-item label="随机名称">
          ...
          <el-form-item label="批量余额">
            <div class="flex items-center gap-3">
              <el-switch v-model="batchEditForm.balanceEnabled" />
              <el-input-number
                v-model="batchEditForm.balance"
                :min="0"
                :step="1000"
                :disabled="!batchEditForm.balanceEnabled"
              />
            </div>
          </el-form-item>
        </el-form>
