...
async function submitPwd() {
  ...
  if (pwdForm.newPassword.length < 6 || pwdForm.newPassword.length > 18) {
    message("新密码长度需 6-18 位", { type: "warning" });
    return;
  }
...
async function openTwofaDialog() {
  ...
  twofaCode.value = "";
  try {
    const statusRes = await getTwofaStatus();
    twofaBound.value = statusRes.data?.bound === true;
    if (!twofaBound.value) {
      const setupRes = await setupTwofa();
...
async function submitTwofaBind() {
  ...
  try {
    await bindTwofa({ secret: twofaSecret.value, code });
    ...
    >
      <div v-loading="twofaLoading">
        <template v-if="twofaBound">
          <p class="twofa-tip">
            当前账号已绑定 Google 验证码。如需更换，请先解绑再重新绑定。
          </p>
        </template>
        <template v-else>
          <p class="twofa-tip">
